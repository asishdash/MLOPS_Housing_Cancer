...
def detect_model_drift(predictions_log, actuals_log, threshold=0.05):
    ...
    predictions_log['Timestamp'] = pd.to_datetime(predictions_log['Timestamp'])
    actuals_log['Timestamp'] = pd.to_datetime(actuals_log['Timestamp'])
    data = pd.merge(predictions_log,actuals_log,on='Timestamp', how='inner')
